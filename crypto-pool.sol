...
contract CryptoPool {
  ...
  string internal constant ADMIN_PERMISSION = "Restricted to Onwer of the contract";
  string internal constant INSUFFICIENT_FUNDS = "Amount to stake should be equal to 0.01 Ether";
  string internal constant CONTRACT_OWNER_WARNING = "Contract Onwer Can not participate in this action";
  ...
  receive() external payable {
   ...
   function addPlayerToList(address _address,uint _amount) PoolOpen private {
  ...
  function getAllCurrentPlayer() public view returns(address payable[] memory){
    return participants;
  }
  ...
  function pickWinner() public  AdminAccess returns(address winnderAddress) {
      ...
      uint randomNumberResult = generateRandomId();
      
      address payable winner;
      
      uint index = randomNumberResult % participants.length;
